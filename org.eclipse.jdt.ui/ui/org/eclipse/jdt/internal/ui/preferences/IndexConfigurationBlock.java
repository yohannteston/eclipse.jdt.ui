...
public class IndexConfigurationBlock {
	...
	private String internalChooseArchivePath() {
		...
		try {
			...
			dialog.setTitle(PreferencesMessages.IndexConfigurationBlock_browse_jarorzip_path_title);
			dialog.setMessage(PreferencesMessages.IndexConfigurationBlock_location_in_jarorzip_message);
	...
	private String chooseArchive() {
		...
		dialog.setText(PreferencesMessages.IndexConfigurationBlock_zipImportSource_title);
	...
	private String chooseWorkspaceArchive() {
		...
		dialog.setTitle(PreferencesMessages.IndexConfigurationBlock_workspace_archive_selection_dialog_title);
		dialog.setMessage(PreferencesMessages.IndexConfigurationBlock_workspace_archive_selection_dialog_description);
	...
	protected void displayErrorDialog(String message) {
		MessageDialog.openError(fShell, PreferencesMessages.IndexConfigurationBlock_error_dialog_title, message);
	}

	private String chooseIndexFile() {
	...
	private IStatus updateURLStatus() {
		...
		try {
			...
			if ("file".equals(url.getProtocol())) { //$NON-NLS-1$
				if (url.getFile() == null) {
					status.setError(PreferencesMessages.IndexConfigurationBlock_error_notafolder);
					return status;
				}
			}
			fURLResult= url;
		} catch (MalformedURLException e) {
			status.setError(PreferencesMessages.IndexConfigurationBlock_MalformedURL_error);
			return status;
		}
		...
		try {
			...
			if (jdocLocation.length() > 0)  {
				if (!Path.ROOT.isValidPath(jdocLocation)) {
					status.setError(PreferencesMessages.IndexConfigurationBlock_error_invalidarchivepath);
					return status;
				}
				if (fWorkspaceRadio.isSelected()) {
					IWorkspaceRoot root= ResourcesPlugin.getWorkspace().getRoot();
					...
					if (res != null) {
						if (!(res instanceof IFile)) {
							status.setError(PreferencesMessages.IndexConfigurationBlock_error_archive_not_found_in_workspace);
							return status;
						}
					} else {
						status.setError(PreferencesMessages.IndexConfigurationBlock_error_archive_not_found_in_workspace);
					...
					if (!path.isAbsolute()) {
						status.setError(PreferencesMessages.IndexConfigurationBlock_error_archivepathnotabsolute);
						return status;
					}
					File jarFile= new File(jdocLocation);
					if (jarFile.isDirectory())  {
						status.setError(PreferencesMessages.IndexConfigurationBlock_error_notafile);
						return status;
					}
					if (!jarFile.exists())  {
						status.setWarning(PreferencesMessages.IndexConfigurationBlock_error_notafile);
					}
				}
